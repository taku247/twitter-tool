...
**`per_execution`** - ツイート取得毎に実行
- 15分毎のCron実行でツイートを取得
- 新しいツイートが最低数（デフォルト5件）以上ある場合に自動実行
- 最も頻繁な分析が可能

**`daily`** - 毎日指定時刻に実行
- `schedule`で指定した時刻（例：`18:00`）に毎日実行
- 前回実行から24時間経過している場合のみ実行


#### スケジュール時刻（schedule）の詳細

**形式**: `"HH:MM"`（24時間形式）
**タイムゾーン**: JST（日本標準時）
**例**: `"18:00"` = 日本時間18時00分

- `frequency`が`daily`の場合に使用
- `per_execution`や`manual`の場合は使用されない（設定として保存されるが無視）
